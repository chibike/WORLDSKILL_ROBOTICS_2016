...
class PolygonObject():
    ...
    def addPoint(self, point):
        ...
        angle = p[0]#because angle is clock wise
        self._angles.append(angle)
        self._lengths.append(length)
        self._quadrants.append(p[1])
        self.update()
    def update(self):
    ...
    def adjust(self, item, x, y, length, angle):
        item_id = self._item_ids[item]
        theta,quadrant = self.calculateNewQuadrant(self._quadrants[item_id], angle)
        dy = length * sin(angle)
        dx = length * cos(angle)
        dx,dy = self.getQuadrant4Alpha(dx, dy, quadrant)
        x2 = x+dx
        y2 = x-dy
