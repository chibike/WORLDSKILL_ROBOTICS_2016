...
class MapScene(QtGui.QGraphicsScene):
    def __init__(self, parent=None, mouseEventsHandler=None):
        super(MapScene, self).__init__(parent)
        self._mouseEventsHandler = mouseEventsHandler
    ...
    def mouseMoveEvent(self, mouseEvent):
        ...
        self._mouseEventsHandler(mouseEvent)
        
        super(MapScene, self).mousePressEvent(mouseEvent)
...
class Ui_Blink_Box_v12(QtGui.QMainWindow):
    ...
    def setup_map(self):
        self.mapWidth = QtGui.QInputDialog.getInteger(self,
                        "Map Properties", "Map Width", 500, 0, 5000)#[2000, True]
        self.mapHeight =QtGui.QInputDialog.getInteger(self,
                        "Map Properties", "Map Height", 200, 0, 5000)#[2000, True]# 
        if self.mapWidth[1] and self.mapHeight[1]:
            self.mapWidth = int(self.mapWidth[0])
            self.mapHeight = int(self.mapHeight[0])
            self.scene.setSceneRect(0, 0, self.mapWidth, self.mapHeight)
            self.graphicsView.setSceneRect(-20, -20, self.mapWidth+20, self.mapHeight+20)
            self.scene.addRect(0, 0, self.mapWidth, self.mapHeight, QtGui.QColor(200, 0, 0), QtGui.QColor(255, 255, 255))
            self.graphicsView.ensureVisible(0, 0, self.mapWidth+30, self.mapHeight+30, 50, 50)
            self.graphicsView.centerOn(self.graphicsView.items()[0])
        else:
            self.mapWidth = 0
            self.mapHeight = 0
    ...
    def setup_scene_and_graphics_view(self):
        ...
        self.scene = MapScene(mouseEventsHandler = self.update_mouse_position)
    ...
    def bottom_widget_creator(self):
        ...
        self.rotateSpinBox = QtGui.QSpinBox(self.widget)
        self.rotateSpinBox.setObjectName(_fromUtf8("rotateSpinBox"))
        self.rotateSpinBox.setMinimum(-360)
        self.rotateSpinBox.setMaximum(360)
        self.rotateSpinBox.setValue(0)
        self.rotateSpinBox.valueChanged.connect(self.update_items_rotation)
        self.gridLayout.addWidget(self.rotateSpinBox, 4, 5, 1, 1)
    ...
    def scene_selection_changed(self):
        if self.scene.selectedItems():
            if self.scene.selectedItems()[0] != self.current_selected_item:
                ...
                self.xPositionSpinBox.setValue(self.scene_object_x)
                self.yPositionSpinBox.setValue(self.scene_object_y)
                #if QtGui.QGraphicsItem.ItemIsMovable in item.flags():
                #    self.lockObjectCheckbox.setCheckState(True)
                #else:
                #    self.lockObjectCheckbox.setCheckState(False)
                print "selected item flags =",item.ItemIsMovable
        else:
            #self.previous_selected_item = self.current_selected_item
            #self.current_selected_item = None
            #self.update_item_selection()
    ...
    def set_qview_zoom2(self, value):
        self.zoom_factor = float(value/100.0)
        if self.previous_zoom_factor != None:
            if self.previous_zoom_factor > self.zoom_factor:
                self.zoom_counter -= (self.previous_zoom_factor - self.zoom_factor)
                self.previous_zoom_factor = self.zoom_factor
                self.graphicsView.scale(1/self.zoom_factor, 1/self.zoom_factor)
                print "zoom_factor =", 1/self.zoom_factor
                return
            elif self.zoom_factor > self.zoom_counter:
                self.zoom_counter = self.zoom_factor
                self.previous_zoom_factor = self.zoom_factor
                self.graphicsView.scale(self.zoom_factor, self.zoom_factor)
                print "zoom_factor =", self.zoom_factor
                return
            elif self.previous_zoom_factor == self.zoom_factor:
                return
    ...
    def width_changed(self):
        #try:
        if self.current_selected_item != None:
            self.current_selected_item.update(self.current_selected_item.x(), self.current_selected_item.y(),
                                              int(self.widthEntry.text()), int(self.heightEntry.text()))
        #except:
        #    self.display_warning("Width Error", "Could not understand width")
    def length_changed(self):
        print "Length Editing Finished, text=",int(self.lengthEntry.text())
        try:
    ...
    def height_changed(self):
        print "Height Editing Finished, text=",int(self.heightEntry.text())
        try:
            value = int(self.heightEntry.text())
        except:
            self.display_error("Height Error", "Could not understand height")
    def update_items_rotation(self):
        rotateVal = int(self.rotateSpinBox.value())
        if self.current_selected_item == None:
            return
        else:
            print "rotation =",self.current_selected_item.rotation()
    ...
    def disable_width_height_properties(self):
        self.strokeWidthSpinBox.setDisabled(True)
        self.strokeHeightSpinBox.setDisabled(True)
    def enable_width_height_properties(self):
        self.strokeWidthSpinBox.setEnabled(True)
        self.strokeHeightSpinBox.setEnabled(True)
    ...
    def draw_rect(self):
        ...
        try:
            #rectItem = getRectangle(int(self.heightEntry.text()), int(self.heightEntry.text()), self.myPen, self.myBrush)
            rectItem = QtGui.QGraphicsRectItem()
            rectItem.setRect(self.scene_object_x, self.scene_object_y, int(self.widthEntry.text()), int(self.heightEntry.text()))
            rectItem.setFlag(QtGui.QGraphicsItem.ItemIsMovable, True)
            rectItem.setFlag(QtGui.QGraphicsItem.ItemIsSelectable, True)
