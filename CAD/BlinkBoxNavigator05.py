...
class Ui_Blink_Box_v12(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.current_selected_item_stroke_properties = 0

        self.selected_stroke_1_type = QtGui.QPen()
        self.selected_stroke_1_type.setColor(QtCore.Qt.red)
        self.selected_stroke_1_type.setWidth(10)
        self.selected_stroke_1_type.setStyle(QtCore.Qt.DashLine)

        self.selected_stroke_2_type = QtGui.QPen()
        self.selected_stroke_2_type.setColor(QtCore.Qt.darkRed)
        self.selected_stroke_2_type.setWidth(10)
    ...
    def setup_scene_and_graphics_view(self):
        ...
        #self.scene.addRect(-30, -30, 60, 60, myPen, myBrush)
        #self.scene.addRect(-30, -30, 60, 60, QtGui.QColor(200, 0, 0), QtGui.QColor(200, 0, 0))
    ...
    def timer_event(self):
        if self.bluetooth_connected == True:
            try:
                sx = self.bluetooth_connectedDevices[0].read()
                if not(sx == ''): 
                    s = self.bluetooth_connectedDevices[0].readline()
    ...
    def scene_selection_changed(self):
        if self.scene.selectedItems():
            if self.scene.selectedItems()[0] != self.current_selected_item:
                ...
                #item.focusItem()
                self.scene.setFocusItem(item, QtCore.Qt.MouseFocusReason)
                self.previous_selected_item = self.current_selected_item
                self.current_selected_item = item
                self.update_item_selection()
                print "selected item type =",type(item)
        else:
            pass
    ...
    def width_changed(self):
        print "Width Editing Finished, text=",str(self.widthEntry.text())
    def length_changed(self):
        print "Length Editing Finished, text=",str(self.lengthEntry.text())
    def radius_changed(self):
        print "Radius Editing Finished, text=",str(self.radiusEntry.text())
    def height_changed(self):
        print "Height Editing Finished, text=",str(self.heightEntry.text())
    def setup_fill(self):
    ...
    def update_items_rotation(self):
        print "rotation =",int(self.rotateSpinBox.value())
