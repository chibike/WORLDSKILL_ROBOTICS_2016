...
class Ui_Blink_Box_v12(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.messageBoxCustomDialog = QMessageBoxCustomDialog(self)
        
        self.mapFilename = "Map File 01.b_map"
        self.mapProfile = "Ackerman Steering"
        self.numberOfMaps = 1
    ...
    def setup_map(self):
        #self.edit_map_properties()
        self.scene.setSceneRect(0, 0, self.dimensions[0]+20, self.dimensions[1]+20)
        self.scene.addRect(0, 0, self.dimensions[0], self.dimensions[1], QtGui.QColor(200, 0, 0), QtGui.QColor(255, 255, 255))
    ...
    def display_popup_menu(self,pos):
        if self.current_selected_item == None:
            ...
            actionEditMapProperties = QtGui.QAction(self)
            actionEditMapProperties.setObjectName(_fromUtf8("actionEditMapProperties"))
            actionEditMapProperties.setText(_translate("Blink_Box_v12", "Edit Map Properties", None))
            #actionEditMapProperties.setShortcut("")
            actionEditMapProperties.setStatusTip("Edit map properties")
            actionEditMapProperties.triggered.connect(self.edit_map_properties)
            #actionEditMapProperties.setIcon(QtGui.QIcon('shapesRectButton01.png'))
            
            actionSelectAll = QtGui.QAction(self)
            actionSelectAll.setObjectName(_fromUtf8("actionSelectAll"))
            actionSelectAll.setText(_translate("Blink_Box_v12", "Select All", None))
            #actionSelectAll.setShortcut("Ctrl+A")
            actionSelectAll.setStatusTip("Select all")
            actionSelectAll.triggered.connect(self.demo_function)
            #actionSelectAll.setIcon(QtGui.QIcon('shapesRectButton01.png'))

            actionHideAll = QtGui.QAction(self)
            actionHideAll.setObjectName(_fromUtf8("actionHideAll"))
            actionHideAll.setText(_translate("Blink_Box_v12", "Hide All", None))
            #actionHideAll.setShortcut("")
            actionHideAll.setStatusTip("Hide all")
            actionHideAll.triggered.connect(self.demo_function)
            #actionHideAll.setIcon(QtGui.QIcon('shapesRectButton01.png'))

            actionShowAll = QtGui.QAction(self)
            actionShowAll.setObjectName(_fromUtf8("actionShowAll"))
            actionShowAll.setText(_translate("Blink_Box_v12", "Show All", None))
            #actionShowAll.setShortcut("")
            actionShowAll.setStatusTip("Show all")
            actionShowAll.triggered.connect(self.demo_function)
            #actionShowAll.setIcon(QtGui.QIcon('shapesRectButton01.png'))
            
            #zoom in/ zoom out
            
            self.rightClickSelectionMenu.addAction(actionEditMapProperties)
            self.rightClickSelectionMenu.addSeparator()
            self.rightClickSelectionMenu.addAction(actionSelectAll)
            self.rightClickSelectionMenu.addAction(actionShowAll)
            self.rightClickSelectionMenu.addAction(actionHideAll)
        ...
        elif isinstance(self.current_selected_item, QtGui.QGraphicsPolygonItem):
            tagData = [i.toString() for i in self.current_selected_item.data(0).toList()]
            if "ItemIsForwardDistance" in tagData:
                pass
            else:
                pass
        else:
    ...
    def mouse_pressed_event(self, event):
        ...
        if event.button() == 2:
            ...
            if self.drawing_polygon_status == True:
                ...
                if self.polyItem.addPoint((self.scene_mouse_x, self.scene_mouse_y)) == 0:
                    self.drawing_polygon_status = False
    ...
    def setup_stroke(self):
        ...
        self.myPen.setStyle(QtCore.Qt.DashDotLine)
        self.myPen.setWidth(5)
        #self.myPen.setBrush(QtCore.Qt.black)
        self.myPen.setCapStyle(QtCore.Qt.RoundCap)
        self.myPen.setJoinStyle(QtCore.Qt.RoundJoin)
        self.strokeWidthSpinBox.setValue(5)
        self.strokeTypeComboBox.setCurrentIndex(1)
    ...
    def item_color_changed(self):
        ...
        self.fillColorToolButton.setIcon(self.create_color_tool_button_icon
                    ( 'floodfill.png', QtGui.QColor(self.fillAction.data()) )
                                         )
    ...
    def add_forward_distance_route(self):
        print "fd50"
        distance = QtGui.QInputDialog.getInteger(self,
                    "Set Distance", "Displacement:", 100, 0, 5000)
        if distance[1]:
            self.textEdit.append("fd,"+str(distance[0])+",")
            self.draw_forward_distance(distance[0])
    def draw_path(self):
        print "Drawing Path"
    ...
    def draw_polygon(self):
        ...
        if self.drawing_polygon_status == True:
            ...
            self.polyItem = PolygonItem(None, self.scene,
                                   self.myPen, self.myBrush)
            self.scene.addItem(self.polyItem)
            self.drawing_polygon_status = True
            self.previous_selected_item = self.current_selected_item
            self.current_selected_item = self.polyItem
            self.scene.clearSelection()
            self.update_item_selection()
    ...
    def draw_line(self):
        ...
        #line.setFocus()
    def draw_forward_distance(self, distance):
        forwardDistanceItem = getForwardDistanceItem(distance, self.myPen, self.myBrush)
        forwardDistanceItem.setPos(self.scene_object_x, self.scene_object_y)
        forwardDistanceItem.setRotation(int(self.rotationSpinBox.value()))
        self.scene.addItem(forwardDistanceItem)
    def find(self):
        ...
        if name:
            ...
            with open(name, 'r') as file:
                ...
                if fileName.endswith('.txt') or fileName.endswith('.bm_script'):
                    ...
                    self.tabWidget.setTabText(self.tabWidget.indexOf(self.Scripts),
                                              _translate("Blink_Box_v12",
                                                         "Scripts - "+fileName.split("/")[-1], None))
                elif fileName.endswith('.b_map'):
                    #try:
                    f = open(fileName, 'r')
                    self.save_buffer = json.load(f)
                    f.close()
                    self.restore_from_save_buffer()
                    #except:
                    #    self.display_warning("File Error", "Could not open file")
                else:
                    print "invalid name"
    ...
    def save_current_tab(self):
        if self.tabWidget.currentWidget() == self.mapTab:
            ...
            self.update_save_buffer()
            try:
                print self.save_buffer
                f = open("test.b_map", 'w')
                json.dump(self.save_buffer, f)
                f.close()
            except NameError():
                self.display_warning("File Error", "Could not Save File")
    ...
    def edit_map_properties(self):
        mapPropertiesDialog = MapSettingsDialog(parent=self, filename=self.mapFilename,
                                                dimensions=self.dimensions)
        mapPropertiesDialog.exec_()
        results = mapPropertiesDialog.results
        if results[-1]:
            self.mapFilename = results[0]
            self.mapProfile = results[1]
            self.numberOfMaps = results[2]
            self.dimensions = results[3]
            self.unit = results[4]
            self.mainMapRect.setRect(0, 0, self.dimensions[0], self.dimensions[1])
            self.scene.setSceneRect(0, 0, self.dimensions[0]+20, self.dimensions[1]+20)

            #--------- Yes Yes Yes I can add Images to the screen and edit it later ---
            #if list(results[3]) == [500, 500]:
            #    print "Drawing Pixmap"
            #    pixmap = QtGui.QPixmap()
            #    pixmap.load("Draw1.jpg")
            #    print pixmap.size()
    ...
    def draw_tab_font_creator(self):
        ...
        self.underlineAction = QtGui.QAction(
                ...
                triggered=self.handle_font_change)
    def update_save_buffer(self):
        self.save_buffer["scene"] = [self.mapProfile, self.numberOfMaps,
                                     self.dimensions, self.unit,
                                     self.scene_mouse_x, self.scene_mouse_y,
                                     self.scene_object_x, self.scene_object_y,
                                     self.route, self.drawing_polygon_status,
                                     self.draw_polygon_buffer,
                                     self.previous_zoom_factor, self.bluetooth_comPort,
                                     self.bluetooth_baudrate, self.bluetooth_connected,
                                     self.bluetooth_connectedDevices, self.deleted_items,
                                     self.current_selected_item, self.previous_selected_item,
                                     self.current_selected_item_stroke_properties]
        self.save_buffer["Pen"] = [self.myPen.style(), self.myPen.width(),
                                   self.myPen.color().name(), self.myPen.capStyle(),
                                   self.myPen.joinStyle()]
        self.save_buffer["brush"] = [QtCore.Qt.black, self.myBrush.gradient(),
                                     self.myBrush.style(), "Texture",
                                     "TextureImage", "Transform"]
        for i in self.scene.items():
            if isinstance(i, QtGui.QGraphicsLineItem):
                pass
            elif isinstance(i, QtGui.QGraphicsPolygonItem):
                pass
            elif isinstance(i, QtGui.QGraphicsRectItem):
                pass
        #print "pen.brush =",self.myPen.brush()
        #print "brush.texture =",self.myBrush.texture()
        #print "brush.textureImage =",self.myBrush.textureImage()
        #print "brush.transform =",self.myBrush.transform()
    def restore_from_save_buffer(self):
        return
        self.scene.setSceneRect(0, 0, self.save_buffer["scene"][2][0]+20,
                                self.save_buffer["scene"][2][1]+20)
        self.mainMapRect.setRect(0, 0, self.save_buffer["scene"][2][0],
                                 self.save_buffer["scene"][2][1])
        self.graphicsView.centerOn(self.mainMapRect)

        self.myBrush.setColor(    self.save_buffer["brush"][0] )
        self.myBrush.setGradient( self.save_buffer["brush"][1] )
        #self.myBrush.setMatrix(   self.save_buffer["brush"][2] )
        self.myBrush.setStyle(    self.save_buffer["brush"][2] )
        #self.myBrush.setTexture(  self.save_buffer["brush"][3] )
        #self.myBrush.setTextureImage( self.save_buffer["brush"][4] )
        #self.myBrush.setTransform( self.save_buffer["brush"][5] )
        
        self.myPen.setStyle( self.save_buffer["pen"][0] )
        self.myPen.setWidth( self.save_buffer["pen"][1] )
        self.myPen.setColor( setColor(QtGui.QColor(self.save_buffer["pen"][2])) )
        self.myPen.setCapStyle( self.save_buffer["pen"][3] )
        self.myPen.setJoinStyle( self.save_buffer["pen"][4] )

    def updateGui(self):
        return
        d = {2: '#000000', 3: '#ffffff', 4: '#808080', 5: '#a0a0a4', 6: '#c0c0c0',
             7: '#ff0000', 8: '#00ff00', 9: '#0000ff', 10: '#00ffff', 11: '#ff00ff',
             12: '#ffff00', 13: '#800000', 14: '#008000', 15: '#000080', 16: '#008080',
             17: '#800080', 18: '#808000', 19: '#000000'}
        n = [i.data().toString() for i in self.fillColorToolButton.menu().actions()]
        print n
        print "dir(self.myBrush.color) =",dir(self.myBrush.color().value)
        print "self.myBrush.color().name()=",self.myBrush.color().name()
        print "dir(self.fillColorToolButton.menu.actions[0]) =",dir(n[0])
        p = [QtCore.Qt.black, QtCore.Qt.white, QtCore.Qt.red, QtCore.Qt.darkRed, QtCore.Qt.blue, QtCore.Qt.darkBlue,
                  QtCore.Qt.yellow, QtCore.Qt.green, QtCore.Qt.darkGreen, QtCore.Qt.cyan, QtCore.Qt.darkCyan,
                  QtCore.Qt.magenta, QtCore.Qt.darkMagenta, QtCore.Qt.darkYellow, QtCore.Qt.gray, QtCore.Qt.darkGray,
                  QtCore.Qt.lightGray, QtCore.Qt.transparent]
        f = [int(i) for i in p]
        print f
        for i in self.fillColorToolButton.menu().actions():
            if QtGui.QColor(i.data().toString()) == self.myBrush.color():
                print "Yeah"
        #print "self.sender =",self.sender()
        #self.item_color_changed()
        #self.text_color_changed()
        #self.line_color_changed()
        #self.set_stroke_type()
        
    def demo_function(self):
        self.display_message("Demo Function", "This fuction is yet to be implemented")
    def display_message(self, title, info):
        self.messageBoxCustomDialog.about(self, title, info)
    def display_warning(self, title, text):
        choice = self.messageBoxCustomDialog.warning(self, title, text)
